...
class OrderExport implements FromCollection, WithHeadings
{
    ...
    */
    ...
    {
        $builder = ActionLog::selectRaw('action_log.product_id, max(p.box_id) as box_id, max(p.box_size) as box_size, max(p.box_weight) as box_weight, max(p.name) as p_name, max(s.count) as s_count, max(pl.count) as pl_count, sum(action_log.count) as al_count, max(c.name) as c_name')
